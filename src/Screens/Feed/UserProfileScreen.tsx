...
const UserProfileScreen = () => {
  ...
  const {getSelectedUserProfile, selectedUserProfile, 
    ...
    userSelectedFollowing, removeFollowing, createFollowUser, userFollowers} = useAuth()
  ...
  return (
    <View className={'flex-1'} style={{backgroundColor: ColorGuide['bg-dark']}}>
      ...
      <View className='p-3 pt-2 flex flex-row items-center justify-between px-10'>
        ...
        <TouchableOpacity onPress={() => {navigation.navigate('UserFollowersScreen')}} className='flex flex-col items-center'>
        ...
        <TouchableOpacity onPress={() => {navigation.navigate('UserFollowingScreen')}} className='flex flex-col items-center'>
      ...
      <ScrollView className='flex-1'>
        {
          selectedUserProfileView === 'posts'
            ? <View className='flex flex-wrap flex-row'>
                {
                  selectedUserPosts?.map((post) => {
                    return(
                      <TouchableOpacity onPress={() => {navigation.navigate('PostDetailsScreenProfile', {post: post})}} className='w-1/3 p-1'>
            ...
            : <View className='flex flex-wrap flex-rcol'>
                {
                  selectedUserLists?.map((list: any) => {
                    return(
                      <TouchableOpacity onPress={() => {navigation.navigate('ListDetailsProfileScreen', {list: list})}} className='w-full p-1 flex flex-row'>
